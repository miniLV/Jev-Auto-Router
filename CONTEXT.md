...
| Codex Orchestration | The single runtime delegation policy layered onto Codex's native multi-agent flow. It supplies Planner, Advisor, and Executor routes and governance; Codex/Root still decides whether delegation is useful and performs any child creation. It is not a separate scheduler. |
| Auto Router | A thin profiling and classification layer. It produces usage insights and a LOW/MEDIUM/HIGH routing hint for Codex Orchestration; it never creates a child Agent and never implements a second delegation decision tree. |
...
| Route Decision | A small, auditable routing hint containing level, phase, reason codes, profile/policy versions, capability mode, and overrides. It is input to the Codex Orchestration policy, not an instruction that independently creates a child Agent. |
...
| Shadow Routing | A non-executing Auto Router evaluation mode. It records the route that Auto Router would recommend while Codex Orchestration continues its normal delegation behavior. It measures classification quality and projected economics without adding another runtime scheduler. |
...
| Personal Usage Dashboard | A local-only, manually refreshed web interface that explains one user's measurable Codex usage habits and data coverage for the current official Credit cycle. It does not upload prompts, code, logs, paths, or task text. |
...
| Usage Attribution Estimate | A model- or effort-level share derived from local Codex session telemetry. It is displayed next to the Official Credit Snapshot, never as an official billed amount. |
