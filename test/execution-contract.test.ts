...
test("applyRoute sets exactly model and effort; tools and input pass through untouched", () => {
  const request = { ...sampleRequest(), input: [{ role: "user", content: "hi" }], tools: [{ type: "shell" }] };
  const { routed, applied } = applyRoute(request, { model: "gpt-5.6-luna", effort: "max" });
  ...
  assert.deepEqual(applied, { original_model: "gpt-5.6-sol", routed_model: "gpt-5.6-luna", routed_effort: "max" });
});

test("observed match: reported model equals routed model", () => {
  const applied = { original_model: "a", routed_model: "gpt-5.6-terra", routed_effort: "medium" };
  ...
  assert.equal(observed.actual_model, "gpt-5.6-terra");
...
test("observed mismatch: reported model differs from routed model", () => {
  const applied = { original_model: "a", routed_model: "gpt-5.6-terra", routed_effort: "medium" };
  const observed = observeExecution(applied, { model: "gpt-5.6-sol" });
...
test("fixture SSE body is valid completed-event content", async () => {
  const stream = sseBody([completedEvent("gpt-5.6-luna")]);
