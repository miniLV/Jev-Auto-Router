...
function html(view: UsageViewModel, nonce: string, loading = false): string {
  ...
  const summary = loading
    ...
    : `<section id="dashboard-summary" class="summary"><article class="note-card"><span class="note-label">Attribution period</span><strong>${escapeHtml(period)}</strong><p>Codex local sessions · UTC calendar boundary</p></article><article class="note-card"><span class="note-label">Official source</span><strong>${officialCredit ? "Available" : "Unavailable"}</strong><p>Credit remains authoritative.</p></article><article class="note-card"><span class="note-label">Top local share</span><strong>${escapeHtml(top?.model ?? "—")}</strong><p>${percent(top?.share ?? 0)} of estimated model credit</p></article></section>`;
  ...
  const readiness = diagnostics.length === 0 ? "" : `<section class="readiness" aria-live="polite"><strong>Setup needed</strong>${diagnostics.map((diagnostic) => `<p><b>${escapeHtml(diagnostic.source === "official" ? "Official Credit" : "Model mix")}:</b> ${escapeHtml(diagnostic.message)} ${escapeHtml(diagnostic.remediation)}</p>`).join("")}</section>`;
