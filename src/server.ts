...
function html(view: UsageViewModel, nonce: string): string {
  ...
  const dynamicStyles = `${MODEL_COLORS.map((color, index) => `.model-swatch[data-color="${index}"]{background:${color}}`).join("")}${officialCredit ? `.credit-progress span{transform:scaleX(${officialCredit.remainingPercent / 100})}.reset-progress span{transform:scaleX(${(reset?.remainingPercent ?? 0) / 100})}` : ""}`;
