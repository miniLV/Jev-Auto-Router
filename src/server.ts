...
function formatCredit(value: string): string {
  ...
  return `${match[1].replace(/\B(?=(\d{3})+(?!\d))/g, ",")}.${(match[2] ?? "").padEnd(2, "0")}`;
}
...
function html(view: UsageViewModel, nonce: string): string {
  ...
  const official = officialCredit
    ? `<div class="hero-value">${escapeHtml(used)} <small>CR</small></div><p>of ${escapeHtml(formatCredit(officialCredit.limit))} CR current-cycle limit</p><div class="progress-label"><span>Credit consumed</span><strong>${usedPercent.toFixed(1)}%</strong></div><div class="progress"><span style="transform:scaleX(${usedPercent / 100})"></span></div><div class="split"><span>Remaining <b>${escapeHtml(remaining)} CR</b></span><span>Reset <b>${escapeHtml(reset?.countdown ?? "—")}</b></span></div><div class="progress-label reset-label"><span>Time remaining to reset</span><strong>${reset?.remainingPercent.toFixed(1) ?? "0"}%</strong></div><div class="progress reset-progress"><span style="transform:scaleX(${(reset?.remainingPercent ?? 0) / 100})"></span></div>`
