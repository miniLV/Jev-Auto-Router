...
function html(view: UsageViewModel, nonce: string): string {
  ...
  const official = officialCredit
    ? `<div class="credit-figure"><span>Official used</span><strong>${escapeHtml(used)} <small>CR</small></strong><p>of ${escapeHtml(displayCredit(officialCredit.limit))} CR limit</p></div><div style="display:flex;justify-content:space-between;margin:0 0 8px;color:#8ea1bc;font-size:11px;letter-spacing:.08em;text-transform:uppercase"><span>Credit used</span><strong>${usedPercent.toFixed(1)}% used · ${escapeHtml(left)} left</strong></div><div class="meter" aria-label="${usedPercent.toFixed(1)}% credit used"><span style="width:${usedPercent}%"></span></div><div class="credit-meta"><div><span>Used</span><strong>${escapeHtml(used)} CR</strong></div><div><span>Remaining</span><strong>${escapeHtml(remaining)} CR</strong></div><div><span>Reset in</span><strong>${escapeHtml(reset?.countdown ?? "—")}</strong></div></div><div style="margin-top:22px"><div style="display:flex;justify-content:space-between;margin:0 0 8px;color:#8ea1bc;font-size:11px;letter-spacing:.08em;text-transform:uppercase"><span>Time to reset</span><strong>${escapeHtml(reset?.countdown ?? "—")} remaining</strong></div><div class="meter" aria-label="${reset?.remainingPercent.toFixed(1) ?? "0"}% of monthly cycle remaining"><span style="width:${reset?.remainingPercent ?? 0}%;background:linear-gradient(90deg,#fbbf24,#fb923c)"></span></div><p style="margin:8px 0 0;color:#8ea1bc;font-size:12px">Monthly reset: ${escapeHtml(officialCredit.resetsAt.slice(0, 10))}</p></div>`
