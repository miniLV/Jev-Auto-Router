...
Relationship: A thin decision layer composed with [the orchestration layer](https://example.invalid/notes/orchestration), not a fork or replacement
...
- **the orchestration layer** supplies role policy and governance. Root still decides whether a plan or child is useful.
...
If a user starts the Root on Sol and a HIGH task needs planning, the orchestration layer still creates the configured Sol Planner as an independent child. V1 intentionally does not invent same-model deduplication: Root and Planner have different contexts and responsibilities even when their model names match.

## 6. Composition with the orchestration layer
...
Root Agent in the user's current task
  validates the decision and chooses whether to delegate
                ↓
the orchestration layer + native multi-agent controls
  enforce Planner / Advisor / Executor role governance
                ↓
Root integrates, verifies, and answers the user
```

the orchestration layer is not a dynamic cost classifier. Auto Router is not another orchestration engine. Root remains the single orchestrator.

### 6.1 Why not use the orchestration layer alone?

the orchestration layer already provides the difficult, reusable role-governance layer:
...
Auto Router must not overwrite the orchestration layer's single-value policy fields independently. If clean coexistence cannot be proven, the release remains analysis/advisory.
...
8. Auto Router and the orchestration layer coexist without policy overwrite.
...
- Thin decision layer on the orchestration layer; no fork and no second scheduler.
...
- Coexistence with the orchestration layer without configuration ownership conflict.
...
6. Can Auto Router coexist with the orchestration layer without overwriting policy?
