...
4. The current installed the orchestration layer skill and its provider/model references.
...
Build a separate, removable `codex-auto-router` plugin that adds a personal usage profile and a thin LOW/MEDIUM/HIGH decision layer on top of the orchestration layer, while keeping the user's current Codex task as the only Root orchestrator.

It is not:

- A company API gateway.
- A budget enforcement service.
- A replacement or fork of the orchestration layer.
- A second scheduler.
...
| the orchestration layer | Planner/Advisor/Executor model and governance policy | Personal usage analysis or cost-tier classification |
...
Auto Router and the orchestration layer cannot safely overwrite the same single-value policy fields independently.
...
| U-004 | Can Auto Router coexist with the orchestration layer without overwriting policy? | Configuration diff and runtime test | Do not enable automatic routing |
...
- Auto Router conflicts with the orchestration layer policy.
...
3. Is a separate plugin justified, or would project instructions plus the orchestration layer be enough for V1?
...
15. Can Auto Router coexist with the orchestration layer without configuration ownership conflict?
