...
The solution must compose with Codex Orchestration rather than replace it. Current capability evidence also differs by Root Model: Terra and Sol can activate the saved multi-agent v2 policy, while Luna uses v1 and does not activate that policy.
...
5. Codex Orchestration remains the role-governance layer.
